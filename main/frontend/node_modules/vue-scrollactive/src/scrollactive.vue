  ...
  computed: {
    ...
    scrollContainer() {
      ...
      if (this.scrollContainerSelector) {
        container =
          document.querySelector(this.scrollContainerSelector) || window;
      }
  ...
  methods: {
    ...
    getItemInsideWindow() {
      ...
      [].forEach.call(this.items, item => {
        ...
        const isScreenPastSection =
          distanceFromTop >= this.getOffsetTop(target) - this.offset;
    ...
    handleClick(event) {
      ...
      this.scrollTo(target).then(() => {
        if (!this.alwaysTrack) {
          this.scrollContainer.addEventListener("scroll", this.onScroll);
          const findClickedItem = item =>
            decodeURI(item.hash.substr(1)) === target.id;
    ...
    scrollTo(target) {
      return new Promise(resolve => {
        ...
        const step = timestamp => {
          ...
          const perTick =
            startingY + easing(progressPercentage) * (difference - offset);
